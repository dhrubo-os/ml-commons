...
public class TransportHybridSearchMemoriesAction extends HandledTransportAction<MLHybridSearchMemoriesRequest, SearchResponse> {
    ...
    private void executeHybridSearch(
        ...
        try {
            ...
            // Only set post_filter if there are actual filter clauses to avoid empty post_filter overhead
            if (postFilter instanceof BoolQueryBuilder && !((BoolQueryBuilder) postFilter).filter().isEmpty()) {
